...
<body class="p-5 flex flex-col text-sm leading-tight text-gray-800">
    @if($invoice->supplier_vat_id)
        <div class="mb-2 text-right font-bold uppercase">{{ __('Invoice - tax document number') }} {{ $invoice->number }}</div>
    @else
        <div class="mb-2 text-right font-bold uppercase">{{ __('Invoice number') }} {{ $invoice->number }}</div>
    @endif
    <div class="flex-1 w-full h-full border border-gray-200 border-collapse">
        <div class="mb-10">
